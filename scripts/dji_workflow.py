...
タイムゾーン:
  - DJI のファイル名 (DJI_YYYYMMDDHHMMSS_...) と MP4 メタデータの
    creation_time は TZ 情報を持たない壁時計のため、Immich 側で
    UTC 扱いされると時刻がずれる。--tz (既定 Asia/Tokyo) を:
      1) Python の filename_to_epoch (= 結合後 MP4 の mtime 計算)
      2) immich-go の --time-zone (= サーバ側での解釈)
    の両方で同じ値として使うことで一貫させる
