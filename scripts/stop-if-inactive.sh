...
#!/bin/bash
set -euo pipefail
CONFIG=$(cat /home/ec2-user/.ec2/autoshutdown-configuration)
...
if is_shutting_down; then
    if [[ ! $SHUTDOWN_TIMEOUT =~ ^[0-9]+$ ]] || is_vfs_connected || is_vscode_connected; then
        sudo shutdown -c
        echo > "/home/ec2-user/.ec2/autoshutdown-timestamp"
    else
        TIMESTAMP=$(date +%s)
        echo "$TIMESTAMP" > "/home/ec2-user/.ec2/autoshutdown-timestamp"
    fi
else
    if [[ $SHUTDOWN_TIMEOUT =~ ^[0-9]+$ ]] && ! is_vfs_connected && ! is_vscode_connected; then
