...
- ここへ来るのは**ログインセッションそのものが切れたとき**だけ。15 分ごとのアクセストークンの
  更新は `aws-login` が refresh token で黙って済ませる。claude の環境には Bedrock 用の
  `AWS_REGION=us-east-1` があるが、`aws-login` はこれを捨てて `-signin` プロファイルのリージョンで
  更新するので、popup に肩代わりさせずに自力で通る
  （[更新はログインしたリージョンでしか通らない](aws-cheatsheet.md#更新はログインしたリージョンでしか通らない)）
