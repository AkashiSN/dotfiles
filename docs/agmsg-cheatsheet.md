...
- リポジトリ直下の `run_onchange_install-agmsg.sh` が `chezmoi apply` 時に公式
  `install.sh` を実行して導入する（`--cmd agmsg` で非対話インストール、既存なら `--update`）。
- ソースは `~/.local/share/agmsg-src` に clone され、本体を更新したいときは
  スクリプト中の `AGMSG_REF` コメントを書き換える（内容が変わるので run_onchange が再実行される）。
