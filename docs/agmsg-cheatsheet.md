...
- `.chezmoiscripts/run_onchange_after_40-ai-assistants.sh.tmpl` の末尾で、claude / codex
  本体の導入に続けて公式 `install.sh` を実行して導入する（`--cmd agmsg` で非対話
  インストール、既存なら `--update`）。agmsg は claude/codex の設定を書き換えるため、
  両者を入れた**後**（同スクリプトの最後）で走らせている。
- ソースは `~/.local/share/agmsg-src` に clone され、本体を更新／固定したいときは
  スクリプト中の `AGMSG_REF` を書き換える（その行・チェックアウト先が変わり、内容変更で
  run_onchange が再実行される）。
...
- `sqlite3` と `git` が必須（macOS は sqlite3 標準搭載、git は before_10 で導入）。
  どちらも無い環境では agmsg の導入だけスキップし、apply は止めない。
