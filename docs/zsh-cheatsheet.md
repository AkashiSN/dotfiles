...
| コマンド | 動作 |
| --- | --- |
| `ide [path...]` | nvim を VSCode ライクな IDE レイアウトで起動（`NVIM_IDE=1 nvim`）。SSH 経由（かつ shpool 外）のときは作業ディレクトリ単位の shpool セッションで包んで切断耐性を付ける（切断後は同じ場所で再度 `ide` すれば復帰）。shpool はデーモンがセッション（nvim）を保持し vt100 状態を記録するので、再アタッチ時に画面を復元する（abduco の PTY 素通しで起きた再接続時の画面崩れ/マウス不作動を解消。デーモンは autodaemonize で自動起動＝systemd 不要）。デタッチキーは既定 `Ctrl-Space Ctrl-q`（2 キー列で誤爆しにくい）、通常は SSH 切断で自動デタッチするので手動デタッチは基本不要。**再アタッチ時は「新規起動と同じ処理」を nvim 側で走らせて立て直す**（RPC/ソケットは使わない）。nvim は起動時に自 pid を `/tmp/nvim-ide-<hash>.pid`（`NVIM_IDE_PIDFILE` で受け取る）へ書き、`ide` 関数が再接続を検知すると `kill -USR1` を送る。nvim は `Signal SIGUSR1` autocmd で `do_reattach()` を起動し、**`:IdeRelayout`（現在サイズで完全に組み直し）→ `set mouse=`→`set mouse=a`（マウス再武装）→ `redraw!`（全面再描画）** を行う。**固定 sleep は使わず**、`SIGUSR1` 受信後 pty resize（`VimResized`）が落ち着くまで `RESIZE_DEBOUNCE_MS`（180ms）デバウンスしてから組み直すので、到着順に依らず最終サイズで正しく組み直る（同 regime の `SIGWINCH` は rebalance しかせず下ターミナル高さ/全体フィットが揃わない=「一ページ」問題、同サイズ復帰では `SIGWINCH` 自体来ない、画面復元はマウス報告 DECSET を再送しない、を一括で補う）。ide-bedrock 用 env は shpool config の `forward_env` で新規セッションへ伝播。`shpool attach` は create-or-attach 一体で、`--cmd` の `zsh -ic` 経由で nvim を起動（デーモンは最小 PATH でセッションを起動するため、zshenv/zshrc をロードして PATH・aqua・node を対話シェルと同一に整えてから exec する）。abduco 自体はまだ導入済みだが ide からは未使用。詳細は [Neovim チートシート](nvim-cheatsheet.md) |
