...
-- 別サイズでの再アタッチやリサイズ後に表示が崩れ / マウスが効かなくなったとき、手動で
-- :Resync を叩いて復旧する（端末多重化ツールの reattach 等で起こりうる）。
-- 原因: nvim は起動時に端末へ DEC private mode 2048(in-band resize)を問い合わせ、対応端末
-- (Ghostty)だと以後サイズを in-band 通知だけで追い、SIGWINCH では追従しなくなる。in-band 通知を
-- 生成しないまま pty サイズだけ変わると、nvim は古いサイズのまま描画し続ける。マウス有効化
-- シーケンスも新しい端末には送られていないので効かない。
-- 対策:
...
--     反映する。端末の応答に依存しないので in-band 非対応の端末(iOS の Termius VT100 など)からでも
--     追従する。reattach 直後は中間サイズを挟むことがあるので数回読み直す。
